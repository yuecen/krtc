...
CalHeatMap.prototype = {
	...
	getSubDomainTitle: function(d) {
		...
		if (d.v === null && !this.options.considerMissingDataAsZero) {
			...
			if (holiday[this.formatDate(new Date(d.t), this.options.subDomainDateFormat)] != null) {
				// console.log(holiday[this.formatDate(new Date(d.t), this.options.subDomainDateFormat)]);
				holiday_name = holiday[this.formatDate(new Date(d.t), this.options.subDomainDateFormat)];
			}
			// console.log(this.formatDate(new Date(d.t), this.options.subDomainDateFormat));
			return (this.options.subDomainTitleFormat.filled).format({
				count: this.formatNumber(value),
				people: this.formatNumber(value.toString().split(".")[1].substring(0, 
						(value.toString().split(".")[1]).length - 1)),
				holiday_name: (holiday_name !== null ) ? holiday_name : '',
